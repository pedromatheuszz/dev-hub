...
import { app, safeStorage, ipcMain, shell, BrowserWindow, nativeTheme } from "electron";
...
class NodeSqliteDriver {
  ...
  all(sql, params) {
    ...
    return params === void 0 ? st.all() : Array.isArray(params) ? st.all(...params) : st.all(params);
  }
  get(sql, params) {
    const st = this.db.prepare(sql);
    return params === void 0 ? st.get() : Array.isArray(params) ? st.get(...params) : st.get(params);
  }
  run(sql, params) {
    const st = this.db.prepare(sql);
    if (params === void 0) st.run();
...
function migrate(db) {
  const atual = db.get("PRAGMA user_version")?.user_version ?? 0;
  let versao = atual;
...
class SourcesRepo {
  ...
  listActive() {
    return this.db.all("SELECT * FROM sources WHERE active = 1 ORDER BY id").map(paraSource);
  }
  markFetched(id, at, etag, lastModified) {
    this.db.run(
...
class ArticlesRepo {
  ...
  existsByCanonicalUrl(url) {
    return this.db.get("SELECT 1 AS x FROM articles WHERE canonical_url = ?", [url]) !== void 0;
  }
  byId(id) {
    const r = this.db.get("SELECT * FROM articles WHERE id = ?", [id]);
    return r ? paraArticle(r) : void 0;
  }
  pendingAi(limit) {
    return this.db.all(
...
class OrderedObjParser {
  constructor(options, externalEntities) {
    ...
    if (this.options.entityDecoder) {
      ...
      this.entityDecoder = new EntityDecoder$1({
...
function detectFormat(body) {
  const head = body.slice(0, 2e3).trimStart();
...
function parseJsonFeed(body) {
  ...
  for (const it of Array.isArray(doc?.items) ? doc.items : []) {
    ...
    const link = it.url ?? it.external_url ?? null;
    if (!title || !link) continue;
...
const MARGEM_FUTURO_MS = 365 * 24 * 60 * 60 * 1e3;
function parseFeedDate(raw, fallback) {
...
const DESCARTAR_CONTEUDO = /* @__PURE__ */ new Set(["script", "style", "noscript", "iframe", "svg"]);
const BLOCO = /* @__PURE__ */ new Set([
  "p",
  "div",
  "br",
  "hr",
...
function htmlToText(html) {
  ...
  const p = new Parser(
    {
      onopentag(nome) {
        if (DESCARTAR_CONTEUDO.has(nome)) {
          ignorando++;
          return;
        }
...
const TAGS_PERMITIDAS = /* @__PURE__ */ new Set([
  "p",
  "br",
  "strong",
  "b",
...
const VAZIAS = /* @__PURE__ */ new Set(["br", "img"]);
  ...
  const p = new Parser(
    {
      ...
      ontext(t) {
        if (ignorando === 0) saida.push(escaparTexto(t));
      },
      onclosetag(nome) {
        if (DESCARTAR_CONTEUDO.has(nome)) {
          if (ignorando > 0) ignorando--;
...
function stableId(canonicalUrl) {
  let h = 2166136261;
...
function normalizeItem(item, source, now) {
  ...
  const excerpt = excerptBase.length > TAMANHO_EXCERPT ? `${excerptBase.slice(0, TAMANHO_EXCERPT).trimEnd()}…` : excerptBase;
  return {
...
function tokenize(text2) {
  return text2.toLowerCase().normalize("NFD").replace(/[̀-ͯ]/g, "").replace(/(\d)\.(?=\d)/g, "$1_").replace(/[^a-z0-9_]+/g, " ").split(" ").filter((t) => t.length > 1);
}
function simhash(text2) {
  const tokens = tokenize(text2);
  if (tokens.length === 0) return "0".repeat(16);
  const pesos = new Array(64).fill(0);
...
function trigrams(text2) {
  const base = tokenize(text2).join("");
...
const HAMMING_MESMO = 3;
const HAMMING_CINZA = 12;
const JACCARD_MESMO = 0.7;
const JACCARD_CINZA = 0.2;
const MAX_PARES_CINZA = 50;
const JANELA_CLUSTER_HORAS = 72;
const MS_POR_HORA$2 = 36e5;
...
function clusterArticles(items) {
  ...
  for (let i = 0; i < items.length; i++) {
    for (let j = i + 1; j < items.length; j++) {
      const distanciaHoras = Math.abs(items[i].publishedAt - items[j].publishedAt) / MS_POR_HORA$2;
  ...
  const cinzas = candidatos.sort((a, b) => b.sim - a.sim).slice(0, MAX_PARES_CINZA).map((c) => c.par);
...
const TERMOS_TECNICOS = /* @__PURE__ */ new Set([
  // linguagens
  "python",
  "javascript",
  "typescript",
...
const PESO_TITULO$1 = 3;
const SATURACAO$1 = 4;
const LIMIAR_RELEVANCIA = 0.4;
function relevanceScore(title, text2) {
  const tokensTitulo = new Set(tokenize(title));
  const tokensTexto = new Set(tokenize(text2));
  if (tokensTitulo.size === 0 && tokensTexto.size === 0) return 0;
  let acertos = 0;
  for (const t of tokensTitulo) {
    if (TERMOS_TECNICOS.has(t)) acertos += PESO_TITULO$1;
  }
  for (const t of tokensTexto) {
    if (tokensTitulo.has(t)) continue;
    if (TERMOS_TECNICOS.has(t)) acertos += 1;
  }
  return acertos / (acertos + SATURACAO$1);
}
function isRelevant(title, text2, limiar = LIMIAR_RELEVANCIA) {
  return relevanceScore(title, text2) >= limiar;
}
...
const POR_SLUG$1 = new Map(TAG_DICTIONARY.map((t) => [t.slug, t]));
function tagsOf(title, text2) {
  const pontos = /* @__PURE__ */ new Map();
  const tokensTitulo = new Set(tokenize(title));
  ...
  for (const token of new Set(tokenize(text2))) {
    if (tokensTitulo.has(token)) continue;
    for (const slug of INDICE.get(token) ?? []) {
      pontos.set(slug, (pontos.get(slug) ?? 0) + PESO_CORPO);
    }
  }
  return [...pontos.entries()].map(([slug, p]) => ({ slug, confidence: Math.min(1, p / SATURACAO) })).sort((a, b) => b.confidence - a.confidence || a.slug.localeCompare(b.slug));
}
function categoryOf(title, text2, hint) {
  const tags = tagsOf(title, text2);
  if (tags.length === 0) return hint ?? "technology";
  const soma = { technology: 0, programming: 0, innovation: 0 };
  for (const { slug, confidence } of tags) {
    const def = POR_SLUG$1.get(slug);
    if (def) soma[def.category] += confidence;
  }
  if (hint) soma[hint] += 0.25;
  let vencedora = "technology";
  for (const c of ["technology", "programming", "innovation"]) {
...
function contentTypeOf(title, text2) {
  const alvo = semAcento(`${title} ${text2.slice(0, 400)}`);
...
function frases(texto) {
  return texto.split(/(?<=[.!?])\s+/).map((f) => f.trim()).filter((f) => f.length >= TAMANHO_MINIMO_FRASE);
}
function extrair(title, texto, quantas) {
  ...
  return pontuadas.sort((a, b) => b.pontos - a.pontos).slice(0, quantas).sort((a, b) => a.indice - b.indice).map((p) => p.frase);
}
...
const MEIA_VIDA_HORAS = {
  technology: 18,
  programming: 18,
  innovation: 36
};
const MS_POR_HORA$1 = 36e5;
function freshness(publishedAt, now, halfLifeHours) {
  const horas = Math.max(0, (now - publishedAt) / MS_POR_HORA$1);
...
const IMPORTANCIA_MINIMA = 0.85;
const ARTIGOS_MINIMOS = 3;
const JANELA_HORAS = 6;
const MS_POR_HORA = 36e5;
function isBreaking(input) {
...
function fonte(id, name, url, feedUrl, kind, categoryHint) {
  return {
    id,
    name,
    url,
...
async function runIngest(deps) {
  ...
  const novos = [];
  const jaVistosNestaRodada = /* @__PURE__ */ new Set();
  for (const fonte2 of ativas) {
    try {
      const res = await platform.http.get({
        url: fonte2.feedUrl,
        etag: fonte2.etag,
        lastModified: fonte2.lastModified,
        timeoutMs: 2e4
      });
      rel.fontesLidas++;
      sources.markFetched(
        fonte2.id,
        agora,
        res.headers["etag"] ?? fonte2.etag,
        res.headers["last-modified"] ?? fonte2.lastModified
      );
      ...
      for (const item of parseFeed(res.body)) {
        rel.itensVistos++;
        const base = normalizeItem(item, fonte2, agora);
        ...
        novos.push({
          ...
          storyId: null,
          contentType: "news",
          // definido pela classificação abaixo
          aiState: "pending"
        });
      }
    } catch (e) {
      rel.fontesComErro++;
      platform.logger.warn(`falha na fonte ${fonte2.id}`, e);
    }
  }
  if (novos.length === 0) return rel;
  for (const a of novos) articles.upsert(a);
  rel.itensNovos = novos.length;
  const classificacoes = /* @__PURE__ */ new Map();
  for (let i = 0; i < novos.length; i += LOTE_IA) {
    const lote = novos.slice(i, i + LOTE_IA).map((a) => {
  ...
  for (const cluster of clusterArticles(entradas)) {
    ...
    const confiavel = membros.some((m) => {
      ...
      return f !== void 0 && (f.kind === "official" || f.trustWeight >= TRUST_CONFIAVEL);
    });
    const historia = {
      id: `st_${primario.id}`,
  ...
  upsert(s) {
    this.db.run(
         ...
         article_count=excluded.article_count`,
      {
        ...
        summary: s.canonicalSummary,
        cat: s.category,
        imp: s.importance,
        brk: s.isBreaking ? 1 : 0,
        first: s.firstSeenAt,
        last: s.lastUpdatedAt,
        count: s.articleCount
      }
    );
  }
  linkArticle(storyId, articleId, isPrimary) {
    this.db.run(
  ...
  topRanked(limit, category, now) {
    const linhas = this.db.all(
      `SELECT st.*,
        ...
        LIMIT :lim`,
      { cat: category, lim: limit * 8 }
      // folga: reordenamos em memória
    );
    return linhas.filter((r) => r.primary_article_id !== null).map((r) => {
      const story = {
        id: r.id,
      ...
      return {
        ...
        breakdown: scoreStory({
          ...
          followMatches: [],
          // Fase 4 preenche isto
          articleCount: story.articleCount
        })
      };
    }).sort((a, b) => b.breakdown.total - a.breakdown.total).slice(0, limit);
  }
}
class TagsRepo {
  constructor(db) {
  ...
  attach(articleId, tagId, confidence, source) {
    this.db.run(
...
function comoTermosLiterais(q) {
  const termos = q.split(/\s+/).map((t) => t.replace(/"/g, "")).filter((t) => t.length > 0);
...
function montarFeed(ctx2, category, limit) {
  const agora = Date.now();
  const salvos = idsSalvos(ctx2);
  return ctx2.stories.topRanked(limit, category, agora).map((r) => {
    const article = ctx2.articles.byId(r.primaryArticleId);
    if (!article) return null;
    return {
      story: r.story,
      article,
      tags: ctx2.tags.tagsFor(article.id),
      breakdown: r.breakdown,
      saved: salvos.has(article.id)
    };
  }).filter((x) => x !== null);
}
function idsSalvos(ctx2) {
  return new Set(
    ctx2.driver.all("SELECT article_id FROM saved_articles").map((r) => r.article_id)
  );
}
function itemDoArtigo(ctx2, article, salvo) {
  const historia = article.storyId ? ctx2.driver.get("SELECT * FROM stories WHERE id = ?", [article.storyId]) : void 0;
  return {
    story: historia ? {
      id: historia.id,
...
function buscar(ctx2, query, limit) {
  const salvos = idsSalvos(ctx2);
  return ctx2.search.query(query, limit).map((id) => ctx2.articles.byId(id)).filter((a) => a !== void 0).map((a) => itemDoArtigo(ctx2, a, salvos.has(a.id)));
}
function listarSalvos(ctx2, limit) {
  return ctx2.driver.all(
    "SELECT article_id FROM saved_articles ORDER BY saved_at DESC LIMIT ?",
    [limit]
  ).map((r) => ctx2.articles.byId(r.article_id)).filter((a) => a !== void 0).map((a) => itemDoArtigo(ctx2, a, true));
}
function alternarSalvo(ctx2, articleId) {
  const existe = ctx2.driver.get(
    "SELECT 1 AS x FROM saved_articles WHERE article_id = ?",
    [articleId]
  );
  if (existe) {
    ctx2.driver.run("DELETE FROM saved_articles WHERE article_id = ?", [articleId]);
    return false;
  }
  ctx2.driver.run(
...
function registrarLeitura(ctx2, articleId) {
  ctx2.driver.run(
    "INSERT INTO reading_history (article_id, opened_at) VALUES (?, ?)",
    [articleId, Date.now()]
  );
}
function listarFontes(ctx2) {
  return ctx2.sources.listActive().map((s) => ({
    id: s.id,
    name: s.name,
    kind: s.kind,
...
function lerConfig(ctx2, key) {
  return ctx2.driver.get(
    "SELECT value FROM settings WHERE key = ?",
    [key]
  )?.value ?? null;
}
function gravarConfig(ctx2, key, value) {
  ctx2.driver.run(
...
const http = {
  async get(req) {
    ...
    const timer = setTimeout(() => ctrl.abort(), req.timeoutMs ?? 2e4);
    try {
      const cabecalhos = {
...
const logger = {
  ...
  error(m, meta) {
    console.error("[erro]", m, meta ?? "");
  }
};
function criarSecretStore(ctx2) {
  const chaveDe = (k) => `secret:${k}`;
  return {
    async get(k) {
      const cifrado = lerConfig(ctx2, chaveDe(k));
    ...
    async set(k, v) {
      ...
      gravarConfig(ctx2, chaveDe(k), safeStorage.encryptString(v).toString("base64"));
    },
    async delete(k) {
      gravarConfig(ctx2, chaveDe(k), "");
    }
  };
}
function criarPlatform(ctx2) {
  return {
    http,
    clock: { now: () => Date.now() },
    logger,
    secrets: criarSecretStore(ctx2)
  };
}
const PROTOCOLOS_SEGUROS = /* @__PURE__ */ new Set(["http:", "https:"]);
const CATEGORIAS = /* @__PURE__ */ new Set(["technology", "programming", "innovation"]);
function validarCategoria(c) {
  return typeof c === "string" && CATEGORIAS.has(c) ? c : null;
}
function validarLimite(n, padrao = 60) {
  const v = Number(n);
  return Number.isFinite(v) && v > 0 ? Math.min(500, Math.floor(v)) : padrao;
}
function validarTexto(s, max = 500) {
  return typeof s === "string" ? s.slice(0, max) : "";
}
function registrarIpc(ctx2) {
  ipcMain.handle("feed", (_e, cat, limite) => montarFeed(ctx2, validarCategoria(cat), validarLimite(limite)));
  ipcMain.handle("saved", (_e, limite) => listarSalvos(ctx2, validarLimite(limite)));
  ipcMain.handle("search", (_e, q, limite) => buscar(ctx2, validarTexto(q), validarLimite(limite, 30)));
  ipcMain.handle("article", (_e, id) => {
    const article = ctx2.articles.byId(validarTexto(id, 64));
    return article ? { article, tags: ctx2.tags.tagsFor(article.id) } : null;
  });
  ipcMain.handle("toggleSaved", (_e, id) => alternarSalvo(ctx2, validarTexto(id, 64)));
  ipcMain.handle("recordRead", (_e, id) => {
    registrarLeitura(ctx2, validarTexto(id, 64));
  });
  ipcMain.handle("sources", () => listarFontes(ctx2));
  ipcMain.handle("getSetting", (_e, k) => lerConfig(ctx2, validarTexto(k, 64)));
  ipcMain.handle("setSetting", (_e, k, v) => {
    gravarConfig(ctx2, validarTexto(k, 64), validarTexto(v, 2e3));
  });
  ipcMain.handle("ingest", async () => runIngest({
    platform: criarPlatform(ctx2),
    sources: ctx2.sources,
    articles: ctx2.articles,
    stories: ctx2.stories,
    tags: ctx2.tags,
    search: ctx2.search,
    ai: new HeuristicProvider()
  }));
  ipcMain.handle("openExternal", async (_e, url) => {
    const bruta = validarTexto(url, 2e3);
    let u;
    try {
      u = new URL(bruta);
    } catch {
...
function criarJanela() {
  ...
  win.webContents.on("will-navigate", (e, url) => {
    const permitida = ehDev ? url.startsWith(process.env["ELECTRON_RENDERER_URL"] ?? "http://localhost") : url.startsWith("file://");
    if (!permitida) {
      e.preventDefault();
      if (url.startsWith("https://") || url.startsWith("http://")) {
        void shell.openExternal(url);
...
function rodarSmokeTest(win) {
  ...
  win.webContents.on("did-finish-load", () => {
    void win.webContents.executeJavaScript(`(() => {
      ...
      clearTimeout(limite);
      const falhas = Object.entries(r).filter(([k, v]) => k.startsWith("tem") || k === "montou" ? !v : false).map(([k]) => k);
      if (falhas.length > 0) {
...
function capturarTela(win, destino) {
